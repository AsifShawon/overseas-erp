...
import { FileText, ShieldCheck, CheckCircle, XCircle, ExternalLink, Download } from "lucide-react";
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium break-all flex items-center gap-1.5">
            ...
            <a href="#" className="text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 inline-flex">
      ...
      <div className="flex items-center gap-2 w-full sm:w-auto justify-end border-t border-slate-50 pt-3 sm:border-t-0 sm:pt-0 dark:border-slate-800">
        {canVerify && document.status !== "VERIFIED" && onVerify && (
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="flex items-center gap-1 rounded bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-700 hover:bg-emerald-100 dark:bg-emerald-950/30 dark:text-emerald-400"
            >
              <CheckCircle className="h-3.5 w-3.5" /> Approve
            </button>
            <button
              onClick={() => onVerify(document.id, "REJECTED")}
              className="flex items-center gap-1 rounded bg-rose-50 px-2.5 py-1 text-[10px] font-bold text-rose-700 hover:bg-rose-100 dark:bg-rose-950/30 dark:text-rose-400"
            >
        ...
        <button className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-slate-500 hover:bg-slate-100 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400 dark:hover:bg-slate-800">
