// src/app/api/company/branches/[id]/suspend/route.ts
...
export async function POST(
  ...
  try {
    ...
    if (!ctx.permissions.includes("SUSPEND_BRANCH")) {
    ...
    if (activeCount <= 1 && targetBranch.status === "ACTIVE") {
      return NextResponse.json({ error: "Cannot suspend the only active branch." }, { status: 400 });
    }
    ...
    await prisma.auditLog.create({
      data: {
        ...
        actionType: "SUSPEND_BRANCH",
        tableName: "Branch",
        recordId: id,
        companyId: ctx.activeCompanyId,
        delta: { status: "SUSPENDED" } as any,
    ...
    console.error("POST /api/company/branches/[id]/suspend Error:", error);
